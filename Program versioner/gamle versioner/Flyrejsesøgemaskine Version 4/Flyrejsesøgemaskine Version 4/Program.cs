...
namespace Flyrejsesøgemaskine_Version_4
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Thread t1 = new Thread(graf.SetAfbudsPris);
    ...
    class Graph
    {
        ...
        public void FjernForbindelse(string a, string b)
        {
            ...
            if (fundet == false)
            {
                Console.WriteLine("Kunne ikke finde forbindelse mellem " + a + " og " + b);
                int c = random.Next(0, byliste.Count);
                int d = random.Next(0, byliste[c].ruteliste.Count); // b er blevet indsat som a's rute.count.
            }

            if (count2 == byliste.Count * (byliste.Count - 1))
            {
                Console.WriteLine("Der er ikke flere forbindelser i grafen..");
            }
